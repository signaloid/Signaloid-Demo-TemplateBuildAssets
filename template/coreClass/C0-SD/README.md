...
on its own (no separate bootloader): it claims the DMA engine, DMA-copies
the writable image from flash to the runtime region, zeroes `.bss` with a
second DMA operation, invalidates the data cache once, runs C++ global
constructors, and calls `main`.
